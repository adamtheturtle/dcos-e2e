...
def _validate_port_map(
    ...
    for ports_definition in value:
        ...
        if not container_port.isdigit():
            message = ('Container port "{container_port}" is an integer.').format(
                container_port=container_port
            )
            raise click.BadParameter(message=message)
        if int(host_port) < 0 or int(host_port) > 65535:
            message = ('Host port "{host_port}" is not a valid port number.').format(
                host_port=host_port
            )
            raise click.BadParameter(message=message)
...
def one_master_host_port_map_option(
