...
def _check_systemd() -> CheckLevels:
    ...
    try:
        ...
        expected = 'bind mount source path does not exist: /sys/fs/cgroup/systemd"'
...
def _check_mount_var() -> CheckLevels:
    ...
    try:
        ...
        expected_docker_machine = ("bind source path does not exist: {source}").format(
            source=source
        )
        if expected in str(exc) or expected_docker_machine in str(exc):
            message = (
...
def doctor() -> None:
    ...
    check_functions = check_functions_no_cluster + check_functions_cluster_needed
