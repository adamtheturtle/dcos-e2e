...
class Jobs(helpers.RetryCommonHttpErrorsMixin, helpers.ApiClientSession):
    ...
    def _http_req_json(self, fn: callable, *args: list, **kwargs: dict) -> dict:
    ...
    def wait_for_run(self, job_id: str, run_id: str, timeout=600):
        ...
        def _wait_for_run_completion(j_id: str, r_id: str) -> bool:
            ...
            if rc.status_code == 404:
                ...
                if history_available:
                    ...
                    log.warning(
                        "Waiting for job run {} to be finished, but history for that job run is not available".format(
                            r_id
                        )
                    )
                    return False
                ...
                raise requests.HTTPError(
                    "Waiting for job run {} to be finished, but getting HTTP status code {}".format(
                        r_id, rc.status_code
                    ),
                    response=rc,
                )
        ...
        try:
            ...
            raise Exception(
                "Job run failed - operation was not "
    ...
    def details(self, job_id: str, history=False) -> dict:
        ...
        """
        url = "{api}/jobs/{job_id}".format(api=self._api_version, job_id=job_id)
    ...
    def destroy(self, job_id: str):
        ...
        """
        url = "{api}/jobs/{job_id}".format(api=self._api_version, job_id=job_id)
        return self._http_req_json(
            self.delete, url, params={"stopCurrentJobRuns": "true"}
        )
    ...
    def start(self, job_id: str) -> dict:
        ...
        """
        url = "{api}/jobs/{job_id}/runs".format(api=self._api_version, job_id=job_id)
