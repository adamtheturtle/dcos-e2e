...
def get_client(
    ...
    return ssh_client.AsyncSshClient(
        ssh.user, ssh.key, targets, parallelism=parallelism, process_timeout=1200
    )
...
def check_results(results: list, node_client, tag: str):
    ...
    for result in results:
        if result["returncode"] != 0:
            log.error(
                "Command failed (exit {}): ".format(result["returncode"])
                + " ".join(result["cmd"])
            )
            log.error("STDOUT: \n" + result["stdout"].decode())
            log.error("STDERR: \n" + result["stderr"].decode())
            log_name = generate_log_filename(
                "{}-{}-journald.log".format(tag, result["host"])
            )
...
def install_dcos(
    ...
    all_client = get_client(
        cluster, "cluster_hosts", node_client, parallelism=parallelism
    )
    ...
    if enable_selinux is not None:
        ...
        check_results(
            all_client.run_command("run", ["sudo setenforce " + setenforce]),
            node_client,
            "Set SELinux mode",
        )
    ...
    if install_prereqs:
        ...
        check_results(
            all_client.run_command(
                "copy", prereqs_script_path, "~/install_prereqs.sh", False
            ),
            node_client,
        ...
        check_results(
            all_client.run_command(
                "run", ["chmod +x ~/install_prereqs.sh", "&&", "~/install_prereqs.sh"]
            ),
            node_client,
...
def prepare_bootstrap(ssh_tunnel: ssh_client.Tunnelled, download_url: str) -> str:
...
def do_genconf(ssh_tunnel: ssh_client.Tunnelled, genconf_dir: str, installer_path: str):
    ...
    ssh_tunnel.command(
        ["sudo", "bash", installer_path, "--genconf"], stdout=sys.stdout.buffer
    )
...
def download_dcos_installer(
    ssh_tunnel: ssh_client.Tunnelled, installer_path: str, download_url: str
):
...
def get_docker_service_status(
    ssh_tunnel: ssh_client.Tunnelled, docker_name: str
) -> str:
    return (
        ssh_tunnel.command(
            [
...
def start_docker_service(
    ssh_tunnel: ssh_client.Tunnelled, docker_name: str, docker_args: list
):
    ssh_tunnel.command(
...
def do_preflight(
    client: ssh_client.AsyncSshClient,
