...
def template_by_instance_type(instance_type):
    if instance_type.split(".")[0] in ("c4", "t2", "m4"):
        template = pkg_resources.resource_string(
            dcos_launch.__name__, "templates/vpc-ebs-only-cluster-template.json"
        )
    else:
        template = pkg_resources.resource_string(
            dcos_launch.__name__, "templates/vpc-cluster-template.json"
        )
...
def param_dict_to_aws_format(user_parameters):
    return [
        {"ParameterKey": str(k), "ParameterValue": str(v)}
...
@retry(
    ...
    wait_exponential_max=20 * 60 * 1000,
...
class BotoWrapper:
    ...
    def get_service_resources(self, service, resource_name):
        ...
        for region in aws_region_names:
            ...
            try:
                yield from getattr(
                    self.resource(service, region["id"]), resource_name
                ).all()
            except ClientError as e:
                if e.response["Error"]["Code"] == "UnauthorizedOperation":
                    log.debug(
                        "Failed getting resources ({}) for region {} with exception: {}".format(
    ...
    def create_stack(
        ...
        if template_body is not None:
            ...
            ), "tempate_body and template_url cannot be supplied simultaneously"
            args["TemplateBody"] = template_body
        else:
            assert (
                template_url is not None
            ), "template_url must be set if template_body is not provided"
    ...
    def create_vpc_tagged(self, cidr, name_tag):
        ...
        vpc_id = ec2.create_vpc(CidrBlock=cidr, InstanceTenancy="default")["Vpc"][
            "VpcId"
        ]
    ...
    def create_internet_gateway_tagged(self, vpc_id, name_tag):
        ...
        gateway_id = ec2.create_internet_gateway()["InternetGateway"][
            "InternetGatewayId"
        ]
        ec2.attach_internet_gateway(InternetGatewayId=gateway_id, VpcId=vpc_id)
        ec2.create_tags(
            Resources=[gateway_id], Tags=[{"Key": "Name", "Value": name_tag}]
        )
    ...
    def create_subnet_tagged(self, vpc_id, cidr, name_tag):
        ...
        subnet_id = ec2.create_subnet(VpcId=vpc_id, CidrBlock=cidr)["Subnet"][
            "SubnetId"
        ]
        ec2.create_tags(
            Resources=[subnet_id], Tags=[{"Key": "Name", "Value": name_tag}]
        )
    ...
    def get_auto_scaling_instances(self, asg_physical_resource_id):
        ...
        return [
            ec2.Instance(i["InstanceId"])
            for asg in self.client("autoscaling").describe_auto_scaling_groups(
...
class CfStack:
    ...
    def wait_for_complete(self, transition_states: list, end_states: list) -> str:
        ...
        def wait_loop():
            ...
            if stack_status in end_states:
                ...
                return stack_status
            log.info(
                "Stack status {status}. Continuing to wait... ".format(
                    status=stack_status
                )
            )
            if stack_status not in transition_states:
                ...
                raise Exception(
                    "StackStatus changed unexpectedly to: {}".format(stack_status)
                )
    ...
    def get_parameter(self, param):
        ...
        raise KeyError(
            "Key not found in template parameters: {}. Parameters: {}".format(
                param, self.stack.parameters
            )
        )
...
class DcosZenCfStack(CfStack):
    ...
    def private_agent_stack(self):
        return PrivateAgentStack(
            self.stack.Resource("PrivateAgentStack").physical_resource_id,
            self.boto_wrapper,
        )
    ...
    def public_agent_stack(self):
        return PublicAgentStack(
            self.stack.Resource("PublicAgentStack").physical_resource_id,
            self.boto_wrapper,
        )
    ...
    def infrastructure(self):
        return CfStack(
            self.stack.Resource("Infrastructure").physical_resource_id,
            self.boto_wrapper,
        )
...
CF_OS_SSH_INFO = {"el7": SSH_INFO["centos"], "coreos": SSH_INFO["coreos"]}

CENTOS_74_WITH_DOCKER_SELINUX_ENFORCING = {
    "ap-northeast-1": "ami-0bc386484490ade7f",
...
OS_AMIS = {
    ...
    "cent-os-7.4-with-docker-selinux-disabled": {
        "ap-northeast-1": "ami-023fe9ba88dfc1339",
    ...
    "cent-os-8.0-with-docker-selinux-permissive": {
        "us-east-1": "ami-0c298469d9dcbba34",
    ...
    "cent-os-7-dcos-prereqs": {
        "us-east-1": "ami-006219aba10688d0b",
...
aws_region_names = [
    ...
    {"name": "South America (Sao Paulo)", "id": "sa-east-1"},
    {"name": "EU (Ireland)", "id": "eu-west-1"},
    {"name": "EU (Frankfurt)", "id": "eu-central-1"},
