...
def run_dcos_engine(dcos_engine_url: str, dcos_engine_template):
    ...
    with open(
        os.path.join(tmpdir, "_output/{}/azuredeploy.json".format(cluster_name)), "r"
    ) as f:
        arm_template = json.load(f)
    with open(
        os.path.join(
            tmpdir, "_output/{}/azuredeploy.parameters.json".format(cluster_name)
        ),
        "r",
    ) as f:
...
class DcosEngineLauncher(dcos_launch.util.AbstractLauncher):
    def __init__(self, config: dict, env=None):
        if env is None:
            azure_subscription_id = dcos_launch.util.set_from_env(
    ...
    def create(self):
        ...
        if windows_image_source_url:
            dcos_engine_template["properties"]["windowsProfile"][
        ...
        arm_template, self.config["template_parameters"] = run_dcos_engine(
            self.config["dcos_engine_tarball_url"], dcos_engine_template
        )  # noqa
        if linux_bs_url:
    ...
    def describe(self):
        return {
            "masters": dcos_launch.util.convert_host_list(
                self.resource_group.get_master_ips()
            ),
