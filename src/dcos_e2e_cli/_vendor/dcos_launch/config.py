...
class LaunchValidator(cerberus.Validator):
    ...
    def __init__(self, *args, **kwargs):
        ...
        assert (
            "config_dir" in kwargs
        ), "This class must be supplied with the config_dir kwarg"
...
def get_validated_config(user_config: dict, config_dir: str) -> dict:
    ...
    validator = LaunchValidator(
        COMMON_SCHEMA, config_dir=config_dir, allow_unknown=True
    )
    ...
    if provider == "onprem":
        ...
        user_config["dcos_config"].setdefault(
            "rexray_config_preset", user_config["platform"]
        )
    ...
    if validator.normalized(user_config).get("auto_set_selinux"):
        if "enable_selinux" in user_config:
            raise Exception(
                "Parameter conflict: enable_selinux cannot be in config if auto_set_selinux is true"
            )

        selinux_compatible = False
        try:
            selinux_compatible = check_selinux_compatible(
                user_config.get("dcos_version", "")
            )
        except Exception:
            pass

        user_config["enable_selinux"] = (
            selinux_compatible
            and "dcos-enterprise" in user_config["installer_url"]
            and user_config["os_name"] == "cent-os-7-dcos-prereqs"
        )
            ...
            validator.schema.update(
                {
                    ...
                    "zen_helper": {"type": "boolean", "default": False},
                }
            )
        validator.schema.update(
            {
            ...
            validator.schema.update(
                {
                    "azure_location": {
                        "type": "string",
                        "required": True,
                        "default_setter": lambda doc: util.set_from_env(
                            "AZURE_LOCATION"
                        ),
                    }
                }
    ...
    if "genconf_dir" in user_config:
        if "dcos_config" in user_config:
            _validate_genconf_scripts(
                user_config["genconf_dir"], user_config["dcos_config"]
            )
...
COMMON_SCHEMA = {
    "provider": {
        "type": "string",
        "required": True,
        "allowed": ["aws", "azure", "dcos-engine", "onprem", "terraform"],
    },
    "config_dir": {"type": "string", "required": False},
    "dcos_version": {
        "type": ["string", "float"],
        "required": False,
        "coerce": lambda version: str(version),
    },
    "launch_config_version": {"type": "integer", "required": True, "allowed": [1]},
    "ssh_port": {"type": "integer", "required": False, "default": 22},
    ...
    "ssh_user": {"type": "string", "required": False, "default": "core"},
    "key_helper": {"type": "boolean", "default": False},
...
TEMPLATE_DEPLOY_COMMON_SCHEMA = {
    ...
    "template_url": {"type": "string", "required": True, "validator": validate_url},
    "template_parameters": {"type": "dict", "required": True},
}
...
def _validate_fault_domain_helper(field, value, error):
    ...
    if not have_primary_region:
        error(
            field,
            "Must have one region declared with `local: true` (i.e. the master region)",
        )
...
ONPREM_DEPLOY_COMMON_SCHEMA = {
    ...
    "installer_url": {"validator": validate_url, "type": "string", "required": True},
    ...
    "num_masters": {"type": "integer", "allowed": [1, 3, 5, 7, 9], "required": True},
    "dcos_config": {
        "type": "dict",
        "required": True,
        "allow_unknown": True,
        ...
        "schema": {
            ...
            "ip_detect_public_filename": {"excludes": "ip_detect_public_contents"},
    ...
    "fault_domain_helper": {
        ...
        "valueschema": {
            "type": "dict",
            "schema": {
                "num_zones": {"required": True, "type": "integer", "default": 1},
                "num_private_agents": {
                    "required": True,
                    "type": "integer",
                    "default": 0,
                ...
                "local": {"required": True, "type": "boolean", "default": False},
            },
        },
        "validator": _validate_fault_domain_helper,
    },
    "prereqs_script_filename": {"type": "string", "default": "install_prereqs.sh"},
    "install_prereqs": {"type": "boolean", "required": False, "default": False},
    "onprem_install_parallelism": {"type": "integer", "required": False, "default": 10},
}


AWS_ONPREM_SCHEMA = {
    "aws_key_name": {"type": "string", "dependencies": {"key_helper": False}},
    "os_name": {
        "type": "string",
    ...
    "instance_device_name": {
        "type": "string",
        "required": True,
        "default_setter": lambda doc: "/dev/xvda"
        if doc["os_name"] == "coreos"
        else "/dev/sda1",
    },
    "bootstrap_instance_ami": {
        "type": "string",
        "required": True,
        "default_setter": lambda doc: aws.OS_AMIS[doc["bootstrap_os_name"]][
            doc["aws_region"]
        ],
    },
    ...
    "bootstrap_instance_device_name": {
        ...
        "required": True,
        "default_setter": lambda doc: "/dev/xvda"
        if doc["bootstrap_os_name"] == "coreos"
        else "/dev/sda1",
    },
    "admin_location": {"type": "string", "required": True, "default": "0.0.0.0/0"},
    "bootstrap_ssh_user": {
        "required": True,
        "type": "string",
    ...
    "aws_block_device_mappings": {
        "required": False,
        "type": "list",
        "valueschema": {"type": "dict", "allow_unknown": True},
    },
    "iam_role_permissions": {
        ...
        "valueschema": {
            "type": "dict",
            "schema": {
                "Resource": {"type": "list", "required": True},
                "Action": {"required": True, "type": "list"},
                "Effect": {"required": True, "allowed": ["Allow", "Deny"]},
            },
        },
    },
}
...
DCOS_ENGINE_SCHEMA = {
    ...
    "windows_admin_password": {"type": "string", "default": "Replacepassword123"},
    ...
    "windows_publisher": {"type": "string", "default": "MicrosoftWindowsServer"},
...
def deduce_image_project(doc: dict):
    ...
    raise util.LauncherError(
        "ValidationError",
        """Couldn't deduce the image project for your source image. Please
...
GCP_ONPREM_SCHEMA = {
    "machine_type": {"type": "string", "required": False, "default": "n1-standard-4"},
    "os_name": {
    ...
    "source_image": {
        "type": "string",
        "required": False,
        "default_setter": lambda doc: "family/"
        + gcp.OS_IMAGE_FAMILIES.get(doc["os_name"], doc["os_name"]),
    },
    "image_project": {
        "type": "string",
        "required": False,
    ...
    "ssh_public_key": {"type": "string", "required": False},
    "disk_size": {"type": "integer", "required": False, "default": 42},
    "disk_type": {"type": "string", "required": False, "default": "pd-ssd"},
    ...
    "disable_updates": {"type": "boolean", "required": False, "default": False},
    "use_preemptible_vms": {"type": "boolean", "required": False, "default": False},
}
...
def get_latest_github_release(org: str, repo: str, default: str):
    try:
        response = requests.get(
            "https://api.github.com/repos/{}/{}/releases/latest".format(org, repo)
        )
        return response.json()["tag_name"][1:]
    except Exception as e:
        log.error(
            "Failed to get latest {} version. Defaulting to {}. Error details: {}".format(
                repo, default, repr(e)
            )
        )
        return default
