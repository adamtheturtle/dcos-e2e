...
class TerraformLauncher(util.AbstractLauncher):
    def __init__(self, config: dict, env=None):
        ...
        self.cluster_profile_path = os.path.join(
            self.init_dir, "desired_cluster_profile.tfvars"
        )
        ...
        try:
            if os.path.exists(self.init_dir):
                raise util.LauncherError(
                    ...
                    "Either the cluster you are trying to create is "
            ...
            try:
                ...
                log.info(
                    "No Terraform installation detected. Terraform is now being installed."
                )
            ...
            if self.config["key_helper"]:
                ...
                log.warning(
                    'WARNING: Since you did not set "key_helper: true" in your config, make sure your '
            ...
            repo = (
                "terraform-dcos-enterprise"
                if self.config["dcos-enterprise"]
                else "terraform-dcos"
            )
            version = (
                self.config["terraform_dcos_enterprise_version"]
                if self.config["dcos-enterprise"]
                else self.config["terraform_dcos_version"]
            )
            module = "github.com/dcos/{}?ref={}/{}".format(
                repo, version, self.config["platform"]
            )
            ...
            with open(self.cluster_profile_path, "w") as file:
                for k, v in self.config["terraform_config"].items():
                    ...
                    if type(k) is dict:
                        ...
                        file.write('"{}"\n'.format(v))
            subprocess.run(
                [self.terraform_cmd(), "init", "-from-module", module],
                cwd=self.init_dir,
                check=True,
                stderr=subprocess.STDOUT,
            )
    ...
    def describe(self) -> dict:
        ...
        result = subprocess.run(
            [self.terraform_cmd(), "output"],
            cwd=self.init_dir,
            check=True,
            stdout=subprocess.PIPE,
        )
        info = result.stdout.decode("utf-8")
        self.config["ssh_user"] = re.search("ssh_user = \w+", info).group(0)[
            len("ssh_user = ") :
        ]

        private_agents_ips = _convert_to_describe_format(
            _get_ips("Private Agent Public IPs", info)
        )
        ...
        description = {
            "bootstrap_host": _convert_to_describe_format(
                _get_ips("Bootstrap Host Public IP", info=info)
            ),
            "masters": _convert_to_describe_format(
                _get_ips("Master Public IPs", info=info)
            ),
            "private_agents": private_agents_ips,
            "public_agents": _convert_to_describe_format(
                _get_ips("Public Agent Public IPs", info)
            ),
        }
        ...
        if public_agent_elb_address:
            description.update(
                {"Public Agent ELB Public IP": public_agent_elb_address[0]}
            )
    ...
    def test(
        ...
        if (
            ...
            or "ssh_user" not in self.config
        ):
            raise util.LauncherError(
                "MissingInput",
        ...
        env_dict = {
            e: "'{}'".format(env_dict[e]) if " " in env_dict[e] else env_dict[e]
            for e in env_dict
        }
...
{env} py.test {args}" """.format(
            env=env_string, args=arg_string
        )
        ...
        return util.try_to_output_unbuffered(
            self.config, test_host, pytest_cmd, test_port
        )
...
class GcpLauncher(TerraformLauncher):
    def create(self):
        ...
        if (
            "gcp_zone" not in self.config["terraform_config"]
            and "GCE_ZONE" in os.environ
        ):
        ...
        if "gcp_project" not in self.config["terraform_config"]:
            with open(self.config["terraform_config"]["gcp_credentials_key_file"]) as f:
                self.config["terraform_config"]["gcp_project"] = json.load(f)[
                    "project_id"
                ]
        return super().create()
    ...
    def key_helper(self):
        if (
            "gcp_ssh_pub_key_file" not in self.config["terraform_config"]
            or "ssh_private_key_filename" not in self.config
        ):
...
class AzureLauncher(TerraformLauncher):
    def create(self):
        ...
        if (
            "azure_region" not in self.config["terraform_config"]
            and "AZURE_LOCATION" in os.environ
        ):
            self.config["terraform_config"]["azure_region"] = util.set_from_env(
                "AZURE_LOCATION"
            )
        return super().create()
    ...
    def key_helper(self):
        if (
            "ssh_pub_key" not in self.config["terraform_config"]
            or "ssh_private_key_filename" not in self.config
        ):
            private_key, public_key = util.generate_rsa_keypair(
                priv_key_format=serialization.PrivateFormat.TraditionalOpenSSL
...
class AwsLauncher(TerraformLauncher):
    def key_helper(self):
        if (
            "ssh_key_name" not in self.config["terraform_config"]
            or "ssh_private_key_filename" not in self.config
        ):
