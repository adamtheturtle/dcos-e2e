    ...
    def command(self, cmd: list, **kwargs) -> bytes:
        ...
        if "stdout" in kwargs:
            return subprocess.run(
                run_cmd, **kwargs, check=True, env={"PATH": os.environ["PATH"]}
            )
        else:
            return subprocess.run(
                run_cmd,
...
class SshClient:
    ...
    ) -> typing.Generator[Tunnelled, None, None]:
...
def parse_ip(ip: str) -> (str, int):
    ...
    if len(tmp) == 2:
        ...
        raise ValueError(
            ...
            + "colon in it. NOTE: IPv6 is not supported at this time. Got: {}".format(
                ip
            )
        )
...
class AsyncSshClient(SshClient):
    ...
    def __init__(
        self, user: str, key: str, targets: list, process_timeout=120, parallelism=10
    ):
    ...
    def start_command_on_hosts(
        ...
        for host in self.__targets:
            ...
            tasks.append(
                asyncio.ensure_future(getattr(self, coroutine_name)(sem, host, *args))
            )
        return tasks
