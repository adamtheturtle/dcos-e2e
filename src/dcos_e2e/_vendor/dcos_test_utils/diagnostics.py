...
class Diagnostics(ARNodeApiClientMixin, RetryCommonHttpErrorsMixin, ApiClientSession):
    ...
    @retrying.retry(
        wait_fixed=2000, stop_max_delay=1200000, retry_on_result=lambda x: x is False
    )
    ...
    def _legacy_wait_for_diagnostics_job(self, last_datapoint: dict):
        ...
        for attributes in response.values():
            assert (
                "is_running" in attributes
            ), "`is_running` field is missing in response"
            assert (
                "job_progress_percentage" in attributes
            ), "`job_progress_percentage` field is missing"
        ...
        if last_datapoint["time"] and last_datapoint["value"]:
            assert (
                datetime.datetime.now() - last_datapoint["time"]
            ) < datetime.timedelta(seconds=15), "Job is not progressing"
    ...
    @retrying.retry(
        stop_max_delay=50000, wait_fixed=2000, retry_on_result=lambda x: x == []
    )
    ...
    def download_diagnostics_reports(
        self, diagnostics_bundles, download_directory=None, master=None
    ):
        ...
        if self.use_legacy_api:
            return self._legacy_download_diagnostics_reports(
                diagnostics_bundles, download_directory, master
            )
        return self._download_diagnostics_reports(
            diagnostics_bundles, download_directory, master
        )

    def _download_diagnostics_reports(
        self, diagnostics_bundles, download_directory, master
    ):
        for bundle in diagnostics_bundles:
            log.info("Downloading {}".format(bundle))
            r = self.get(
                os.path.join("/diagnostics/", bundle, "file"), stream=True, node=master
            )
    ...
    def _legacy_download_diagnostics_reports(
        self, diagnostics_bundles, download_directory, master
    ):
        for bundle in diagnostics_bundles:
            log.info("Downloading {}".format(bundle))
