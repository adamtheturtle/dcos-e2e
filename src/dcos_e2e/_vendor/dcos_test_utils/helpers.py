...
def check_json(response: requests.Response):
    ...
    try:
        ...
        logging.exception(
            "Could not deserialize response contents:{}".format(
...
class Url:
    ...
    def copy(
        self, scheme=None, host=None, path=None, query=None, fragment=None, port=None
    ):
    ...
    def api_request(
        ...
        **kwargs
        ...
        if node is not None:
            assert (
                port is None
            ), "node is intended to retrieve port; cannot set both simultaneously"
            assert (
                host is None
            ), "node is intended to retrieve host; cannot set both simultaneously"
            if node in self.masters:
                ...
                raise Exception(
                    "Node {} is not recognized within the DC/OS cluster".format(node)
                )
            host = node
