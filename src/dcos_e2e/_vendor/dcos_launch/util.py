...
json_prettyprint_args = {"sort_keys": True, "indent": 2, "separators": (",", ":")}
...
def get_temp_config_path(tmpdir, name, update: dict = None):
    config = yaml.load(
        pkg_resources.resource_string(
            dcos_launch.__name__, "sample_configs/{}".format(name)
        ).decode("utf-8")
    )
...
class AbstractLauncher(metaclass=abc.ABCMeta):
    def get_ssh_client(self, user="ssh_user"):
        return dcos_test_utils.ssh_client.SshClient(
            self.config[user], self.config["ssh_private_key"]
        )
    ...
    def test(
        ...
        if args is None:
            args = list()
        if (
            self.config["ssh_private_key"] == NO_TEST_FLAG
        ...
        if dcos_version:
            env_dict["DCOS_CLI_URL"] = (
                ...
                else "https://downloads.dcos.io/binaries/cli/linux/x86-64/dcos-{}/dcos".format(
                    dcos_version
                )
            )
        ...
        if "MASTER_HOSTS" not in env_dict:
            env_dict["MASTER_HOSTS"] = ",".join(
                m["private_ip"] for m in details["masters"]
            )
        if "PUBLIC_MASTER_HOSTS" not in env_dict:
            env_dict["PUBLIC_MASTER_HOSTS"] = ",".join(
                m["private_ip"] for m in details["masters"]
            )
        if "SLAVE_HOSTS" not in env_dict:
            env_dict["SLAVE_HOSTS"] = ",".join(
                m["private_ip"] for m in details["private_agents"]
            )
        if "PUBLIC_SLAVE_HOSTS" not in env_dict:
            env_dict["PUBLIC_SLAVE_HOSTS"] = ",".join(
                m["private_ip"] for m in details["public_agents"]
            )
        if "DCOS_DNS_ADDRESS" not in env_dict:
            env_dict["DCOS_DNS_ADDRESS"] = (
                "http://" + details["masters"][0]["private_ip"]
            )
...
{env} py.test {args} | tee ~/pytest_output" """.format(
...
def try_to_output_unbuffered(info, test_host: str, bash_cmd: str, port: int) -> int:
    ...
    """
    ssh_client = dcos_test_utils.ssh_client.SshClient(
        info["ssh_user"], info["ssh_private_key"]
    )
    ssh_client.wait_for_ssh_connection(test_host, port=port)
    try:
        ssh_client.command(
            test_host, ["bash", "-c", bash_cmd], port=port, stdout=sys.stdout.buffer
        )
...
def generate_rsa_keypair(
    key_size=2048, priv_key_format=serialization.PrivateFormat.PKCS8
):
    ...
    private_key = rsa.generate_private_key(
        public_exponent=65537, key_size=key_size, backend=crypto_backend
    )
