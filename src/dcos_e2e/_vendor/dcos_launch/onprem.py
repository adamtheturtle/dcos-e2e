        ...
        * adding fault domain real or logical script
        ...
        if "master_list" not in onprem_config:
            onprem_config["master_list"] = json.dumps(
                [h.private_ip for h in cluster.masters]
            )
        ...
        if (
            exhibitor_backend == "zookeeper"
            and "exhibitor_zk_hosts" not in onprem_config
        ):
            zk_service_name = "dcos-bootstrap-zk"
            with self.get_bootstrap_ssh_client().tunnel(
                cluster.bootstrap_host.public_ip
            ) as t:
            ...
            onprem_config["exhibitor_zk_hosts"] = (
                cluster.bootstrap_host.private_ip + ":2181"
            )
        ...
        for script in ("ip_detect", "ip_detect_public", "fault_domain_detect"):
            ...
            script_path = os.path.join(
                genconf_dir, onprem_config.get(filename_key, script_hyphen)
            )
            ...
            if filename_key in onprem_config:
                if os.path.isabs(onprem_config[filename_key]) and os.path.exists(
                    onprem_config[filename_key]
                ):
                    continue
            ...
            shutil.copyfile(
                pkg_resources.resource_filename(
                    ...
                    script_hyphen + "/{}.sh".format(self.config["platform"]),
                ),
                default_script_path,
            )
        ...
        with open(os.path.join(genconf_dir, "config.yaml"), "w") as f:
            if "ip_detect_contents" not in onprem_config:
                ...
                onprem_config["ip_detect_public_filename"] = os.path.join(
                    os.pardir, "genconf", "ip-detect-public"
                )
        ...
        for region, info in self.config["fault_domain_helper"].items():
            ...
            if info["local"]:
                while len(masters) > 0:
                    hostname = (
                        self.get_ssh_client()
                        .command(masters.pop().public_ip, ["hostname"])
                        .decode()
                        .strip("\n")
                    )
            ...
            for _ in range(info["num_public_agents"]):
                if len(public_agents) > 0:
                    ...
                    hostname = (
                        self.get_ssh_client()
                        .command(public_agents.pop().public_ip, ["hostname"])
                        .decode()
                        .strip("\n")
                    )
                    region_zone_map[hostname] = region + "-" + str(zones[z_i % z_mod])
            ...
            for _ in range(info["num_private_agents"]):
                if len(private_agents) > 0:
                    ...
                    hostname = (
                        self.get_ssh_client()
                        .command(private_agents.pop().public_ip, ["hostname"])
                        .decode()
                        .strip("\n")
                    )
                    region_zone_map[hostname] = region + "-" + str(zones[z_i % z_mod])
            ...
            for host, zone in region_zone_map.items():
                case_str += case_template.format(
                    hostname=host, region=region, zone=zone
                )
...
"""
    ...
    def install_dcos(self):
        ...
        with bootstrap_ssh_client.tunnel(bootstrap_host) as t:
            installer_path = platforms_onprem.prepare_bootstrap(
                t, self.config["installer_url"]
            )
            complete_config = self.get_completed_onprem_config()
            platforms_onprem.do_genconf(t, self.config["genconf_dir"], installer_path)

        prereqs_script_path = config.expand_path(
            pkg_resources.resource_filename(
                dcos_launch.__name__,
                "scripts/" + self.config["prereqs_script_filename"],
            ),
