...
class DcosCloudformationLauncher(util.AbstractLauncher):
    def __init__(self, config: dict, env=None):
        self.boto_wrapper = aws.BotoWrapper(config["aws_region"])
        self.config = config
    ...
    def create(self):
        ...
        self.config.update(
            {"stack_id": stack.stack_id, "temp_resources": temp_resources}
        )
        return self.config
    ...
    def zen_helper(self):
        ...
        if "Vpc" not in parameters:
            vpc_id = self.boto_wrapper.create_vpc_tagged(
                "10.0.0.0/16", self.config["deployment_name"]
            )
            parameters["Vpc"] = vpc_id
            temp_resources["vpc"] = vpc_id
        if "InternetGateway" not in parameters:
            gateway_id = self.boto_wrapper.create_internet_gateway_tagged(
                vpc_id, self.config["deployment_name"]
            )
    ...
    def describe(self):
        return {
            ...
            "private_agents": util.convert_host_list(
                self.stack.get_private_agent_ips()
            ),
    ...
    def delete(self):
        ...
        if status != "DELETE_COMPLETE":
            log.info(
                "Deletion was interrupted by another operation (most likely tagging). Retrying to delete..."
            )
            self.delete()
...
class OnPremLauncher(DcosCloudformationLauncher, onprem.AbstractOnpremLauncher):
    ...
    def create(self):
        ...
        template_parameters = {
            ...
            "BootstrapInstanceDeviceName": self.config[
        ...
        if "aws_block_device_mappings" in self.config:
            ...
            template_body_json["Resources"]["BareServerLaunchConfig"]["Properties"][
                "BlockDeviceMappings"
            ].extend(self.config["aws_block_device_mappings"])
        if "iam_role_permissions" in self.config:
            template_body_json["Resources"]["BareRole"]["Properties"]["Policies"][0][
                "PolicyDocument"
            ]["Statement"].extend(self.config["iam_role_permissions"])
        self.config.update(
            {
    ...
    def get_onprem_cluster(self):
        ...
        def _get_cluster():
            ...
            cluster_matches_config = (
                num_masters_found == num_masters
                and num_private_agents_found == num_private_agents
                and num_public_agents_found == num_public_agents
                and bootstrap_host_found
            )

            if not cluster_matches_config:
                msg = (
                    "Not all required hosts were found:\n"
                    "\tmaster count: expected {}, found {}\n"
                    "\tprivate agent count: expected {}, found {}\n"
                    "\tpublic agent count: expected {}, found {}\n"
                    "\tbootstrap host found: {}".format(
                        num_masters,
                        num_masters_found,
                        num_private_agents,
                        num_private_agents_found,
                        num_public_agents,
                        num_public_agents_found,
