...
class AzureResourceGroupLauncher(dcos_launch.util.AbstractLauncher):
    ...
    def describe(self):
        return {
            ...
            "private_agents": dcos_launch.util.convert_host_list(
                self.resource_group.get_private_agent_ips()
            ),
            "public_agents": dcos_launch.util.convert_host_list(
                self.resource_group.get_public_agent_ips()
            ),
    ...
    def key_helper(self):
        ...
        self.config["template_parameters"].update(
            {"sshRSAPublicKey": public_key.decode()}
        )

    @property
    def resource_group(self):
        try:
            return dcos_launch.platforms.arm.DcosAzureResourceGroup(
